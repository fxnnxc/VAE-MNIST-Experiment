...
class VAE(nn.Module):
    ...
    def __init__(self,
        ...
        self.musig_enc = nn.Sequential(
            Linear(int(n), int(n/2), xavier=True),
            nn.ReLU(),
    ...
    def decoder_vae(self, prev_output_tokens, encoder_out, incremental_state=None):
        ...
        for dnum in range(5):
            vis.line(X=[self.count], Y=[mean_enc[0][dnum].item()], win="Mu"+str(dnum),  name="enc_mu",  update='append',opts=dict(title="enc_MU"+str(dnum), showlegend=True))
            vis.line(X=[self.count], Y=[lvar_enc[0][dnum].item()], win="SIG"+str(dnum), name="enc_lvar",  update='append',opts=dict(title="enc_LVAR"+str(dnum), showlegend=True))
            vis.line(X=[self.count], Y=[mean_dec[0][dnum].item()], win="Mu"+str(dnum),  name="dec_mu", update='append',opts=dict(title="dec_MU"+str(dnum), showlegend=True))
            vis.line(X=[self.count], Y=[lvar_dec[0][dnum].item()], win="SIG"+str(dnum),  name="dec_lvar",  update='append',opts=dict(title="dec_LVAR"+str(dnum), showlegend=True))
        self.count += 1
